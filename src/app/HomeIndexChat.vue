...
<template>
  <div
    @click="isShow = true"
    :class="`
      fixed right-4 bottom-14 transition-transform duration-300 border border-link-300 rounded-full z-50
  ...
  <div
    :class="`
      fixed z-50 right-4 bottom-0
      w-[480px] h-[600px]
      flex transition-transform duration-300 mb-4 shadow-lg border border-light-100 rounded-lg
  ...
  >
    <div class=" rounded-lg flex flex-col bg-transparent overflow-hidden">
      ...
      <div class="flex-auto bg-light-100 bg-opacity-50 overflow-auto space-y-4 px-4 py-4 backdrop-blur">
      ...
      <footer class="h-14 w-full bg-white shrink-0 flex items-center justify-center">
        ...
        <button
          class="
            bg-link-500 border border-link-500
            py-1.5 w-20 rounded-r text-sm text-white
